...
export const StaffPerformanceModal = ({ isOpen, onClose, staff, performance }) => {
  ...
  const dailyData = performance.daily?.length ? performance.daily : performance.dailyBreakdown || [];
  const weeklyData = performance.weekly || {};
  const monthlyData = performance.monthly?.totalSales !== undefined ? performance.monthly : performance.monthlyPerformance || {};
  const yearlyData = performance.yearly?.totalSales !== undefined ? performance.yearly : performance.yearlyPerformance || {};
...
const MetricTable = ({ rows, variant = 'neutral' }) => (
  ...
  const summaryRows = [
    ...
    { label: "Monthly Revenue", value: formatCurrency(monthlyData.totalAmount || 0) },
    { label: "Yearly Items Sold", value: yearlyData.itemsSold || 0 },
    { label: "Monthly Avg Sale", value: formatCurrency(monthlyData.avgSaleAmount || 0) },
  ];
  ...
  const weeklyRows = [
    ...
    { label: "Revenue", value: formatCurrency(weeklyData.totalAmount || 0) },
    { label: "Items Sold", value: weeklyData.itemsSold || 0 },
    { label: "Avg Sale", value: formatCurrency(weeklyData.avgSaleAmount || 0) },
  ];

  const monthlyRows = [
    { label: "Total Sales", value: monthlyData.totalSales || 0 },
    { label: "Revenue", value: formatCurrency(monthlyData.totalAmount || 0) },
    { label: "Items Sold", value: monthlyData.itemsSold || 0 },
    { label: "Avg Sale", value: formatCurrency(monthlyData.avgSaleAmount || 0) },
  ];

  const yearlyRows = [
    { label: "Total Sales", value: yearlyData.totalSales || 0 },
    { label: "Revenue", value: formatCurrency(yearlyData.totalAmount || 0) },
    { label: "Items Sold", value: yearlyData.itemsSold || 0 },
    { label: "Avg Sale", value: formatCurrency(yearlyData.avgSaleAmount || 0) },
  ];
