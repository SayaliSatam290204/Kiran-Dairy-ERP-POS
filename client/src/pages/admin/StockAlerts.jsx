...
export const StockAlerts = () => {
  ...
  const quickDispatch = async (request) => {
    if (dispatchingRequestId === request._id) return;
    setDispatchingRequestId(request._id);
    ...
    try {
      ...
      const updateResponse = await adminApi.updateRestockRequestStatus(request._id, { status: 'approved' });

      if (!updateResponse || !updateResponse.data) {
        throw new Error('Failed to update restock request status after dispatch');
      }

      toast.success(`Quick dispatch created for ${request.requestedQty} units to ${request.shopId.name}`);
      fetchRestockRequests(filterShop);
    } catch (error) {
      toast.error('Failed to create dispatch or approve request');
  ...
  const rejectRequest = async (request) => {
    ...
    try {
      const response = await adminApi.updateRestockRequestStatus(request._id, { status: 'rejected' });
      if (!response || !response.data) {
        throw new Error('Failed to reject restock request');
      }
      toast.success(`Restock request rejected for ${request.productId?.name || 'product'}`);
      fetchRestockRequests(filterShop);
    } catch (error) {
  ...
  const fulfillRequest = async (request) => {
    ...
    try {
      const response = await adminApi.updateRestockRequestStatus(request._id, { status: 'fulfilled' });
      if (!response || !response.data) {
        throw new Error('Failed to fulfill restock request');
      }
      toast.success(`Restock request fulfilled for ${request.productId?.name || 'product'}. Shop can now send new requests.`);
      fetchRestockRequests(filterShop);
    } catch (error) {
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                ...
                <div>
                  ...
                  <div className="font-bold text-lg">
                    {activeTab === 'requests'
                      ? item.requestedQty
                      : item.pendingRequestQty ?? '—'}
                  </div>
                </div>
                <div>
                  <div className="text-xs text-gray-600 uppercase tracking-wide">Date</div>
                  <div className="text-sm">
                    {formatDate(item.pendingRequestDate || item.createdAt || item.lastUpdated)}
                  </div>
                </div>
              </div>
              ...
              {activeTab === 'requests' && item.status === 'pending' && (
                <div className="flex gap-3 pt-4 border-t">
                  <button
                    ...
                    disabled={dispatchingRequestId === item._id}
                    className={`flex-1 py-3 px-6 rounded-lg font-semibold flex items-center gap-2 justify-center shadow-lg transition-all ${
                      dispatchingRequestId === item._id
                  ...
                  >
                    <FaTruck />
                    {dispatchingRequestId === item._id ? 'Dispatching...' : 'Quick Dispatch'}
                  </button>
                  <button
