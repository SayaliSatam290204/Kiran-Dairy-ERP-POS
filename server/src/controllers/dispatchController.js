...
export const dispatchController = {
  create: async (req, res) => {
    try {
      ...
      // Prevent duplicate dispatch if the same request is submitted twice rapidly
      if (!isBatchDispatch && dispatchData.dispatchNotes) {
        const duplicate = await Dispatch.findOne({
        ...
        if (duplicate) {
          return responseHelper.error(
            res,
            'Duplicate dispatch detected. Please wait a moment before retrying.',
            409
          );
        }
      }
