...
export const getDashboard = async (req, res) => {
  try {
    const { days = 30, limit = 10, startDate, endDate, selectedBranches } = req.query;

    console.log(
      `[DASHBOARD] Loading data for days=${days}, startDate=${startDate}, endDate=${endDate}, selectedBranches=${selectedBranches}, limit=${limit}`
    );
    ...
    if (selectedBranches) {
      if (typeof selectedBranches === "string") {
        try {
          parsedBranches = JSON.parse(selectedBranches);
        } catch {
          parsedBranches = selectedBranches.split(",");
        }
      } else if (Array.isArray(selectedBranches)) {
        parsedBranches = selectedBranches;
      }
    }
    ...
    const data = await getSuperAdminDashboard({
      ...
      startDate,
      endDate,
      selectedBranches: parsedBranches,
    });
